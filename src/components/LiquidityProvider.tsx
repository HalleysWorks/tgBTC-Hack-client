...
export default function LiquidityProvider() {
  ...
  const [selectedToken, setSelectedToken] = useState<PoolToken | null>(tokens[0] || null);
  ...
  useEffect(() => {
    const loadPoolData = async () => {
      ...
      try {
        ...
        const dedustPool = DeDustPoolContract.fromAddress(addresses.dedustPool, client);
  ...
  const handleAddLiquidity = async () => {
    ...
    try {
      ...
      if (selectedToken.contract === 'vault') {
        ...
        // Send to DeDust pool
        const pool = DeDustPoolContract.fromAddress(addresses.dedustPool, client!);
      ...
      {isWalletConnected && selectedToken && (
        <div className='bg-white dark:bg-gray-900 rounded-2xl p-8 shadow-sm border border-gray-200 dark:border-gray-800'>
          ...
          <div className='space-y-4 mb-6'>
            ...
            <div className='flex justify-between text-sm'>
              ...
              <span className='text-gray-900 dark:text-white font-medium text-xs'>
                {selectedToken.contract === 'dedust' ? addresses?.dedustPool.slice(0, 8) + '...' : addresses?.vault.slice(0, 8) + '...'}
              </span>
            </div>
          </div>
          ...
          <p className='text-xs text-gray-500 dark:text-gray-400 mt-4 text-center'>
            By adding liquidity, you agree to the pool's terms. Your transaction will be sent to the {selectedToken.contract} contract.
          </p>
        </div>
      )}
    </div>
